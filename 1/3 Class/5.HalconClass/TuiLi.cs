 ...
 *
 * 文 件 名:   TuiLi.cs
 * 描    述: 该文件只用于辅助深度学习语义分割推理前的图像预处理
 * 
 * 版    本：  V1.0
 * 创 建 者：  风云
 * 创建时间：  2023年4月19日19:14:41
 * ======================================
 * 历史更新记录
 * 版本：V          修改时间：         修改人：
 * 修改内容：
...
public partial class HDevelopExport
{
  ...
  public void dev_display_ok_nok_COPY_1 (HTuple hv_Areas, HTuple hv_WindowHandleImage)
  {
  ...
  private void action()
  {
    ...
    HObject ho_ROI_0, ho_Image=null, ho_SegmentationImage=null;
    HObject ho_ClassRegions=null, ho_ClassRegion=null, ho_RegionIntersection=null;
    HObject ho_ConnectedRegions=null, ho_SelectedRegions=null;
    HObject ho_RegionFillUp=null;

    // Local control variables 

    HTuple hv_WindowHandle = new HTuple(), hv_ExampleDataDir = new HTuple();
    HTuple hv_PreprocessParamFileName = new HTuple(), hv_RetrainedModelFileName = new HTuple();
    HTuple hv_DictHandle = new HTuple(), hv_ClassNames = new HTuple();
    HTuple hv_ClassIDs = new HTuple(), hv_BatchSizeInference = new HTuple();
    HTuple hv_UseGPU = new HTuple(), hv_WindowHandle1 = new HTuple();
    ...
    HTuple hv_ClassID1 = new HTuple(), hv_ClassID2 = new HTuple();
    HTuple hv_ClassID3 = new HTuple(), hv_ClassID4 = new HTuple();
    HTuple hv_ClassID5 = new HTuple(), hv_ClassID6 = new HTuple();
    HTuple hv_MinArea = new HTuple(), hv_ImageFiles = new HTuple();
    HTuple hv_Index = new HTuple(), hv_DLSampleBatch = new HTuple();
    HTuple hv_DLResultBatch = new HTuple(), hv_Areas = new HTuple();
    HTuple hv_AreaTotals = new HTuple(), hv_ClassIndex = new HTuple();
    ...
    try
    {
      ...
      //加载预处理模型参数文件、训练的最优模型、以及语义分割导出的标注文件
      hv_ExampleDataDir.Dispose();
      hv_ExampleDataDir = "New_Model";
      hv_PreprocessParamFileName.Dispose();
      using (HDevDisposeHelper dh = new HDevDisposeHelper())
      {
      ...
      HOperatorSet.ReadDict("D:/Project/1.正负极极片缺陷检测/训练模型/2022-10-10模型和程序/New_Model/负极极片缺陷语义分割.hdict", 
          new HTuple(), new HTuple(), out hv_DictHandle);
      //从语义分割导出的文件当中获取分割的序号以及类别索引
      hv_ClassNames.Dispose();
      ...
      //set_dict_tuple (DictHandle, 'class_MinArea', [100,10,300,10,100,10,10])
      //set_dict_tuple (DictHandle, 'class_MaxArea', [599999,599999,100000,599999,9999999,9999999,987654321])
      //set_dict_tuple (DictHandle, 'class_BeforArea', [0,0,0,0,0,0,0])
      //set_dict_tuple (DictHandle, 'class_Open', [false,false,false,false,false,false,true])
      //write_dict (DictHandle, 'D:/负极分割.hdict', [], [])
      //read_dict ('D:/负极分割.hdict', [], [], DictHandle1)
      ...
      using (HDevDisposeHelper dh = new HDevDisposeHelper())
      {
      hv_MinArea = new HTuple();
      hv_MinArea = hv_MinArea.TupleConcat(hv_ClassID1, hv_ClassID2, hv_ClassID3, hv_ClassID4, hv_ClassID5, hv_ClassID6);
      }
      if (HDevWindowStack.IsOpen())
      {
        HOperatorSet.SetDraw(HDevWindowStack.GetActive(), "margin");
      ...
      //ROI
      //gen_rectangle1 (ROI_0, 5.21712, -43.5957, 487.996, 1055.16)
      //gen_rectangle1 (ROI_0, 28.0856, 37.6998, 466.398, 794.764)
      ho_ROI_0.Dispose();
      HOperatorSet.GenRectangle1(out ho_ROI_0, 1, 1, 1222, 1022);
      ...
      hv_ImageFiles.Dispose();
      HOperatorSet.ListFiles("D:/Project/1.正负极极片缺陷检测/OK", (new HTuple("files")).TupleConcat(
          ...
          ))-1); hv_Index = (int)hv_Index + 1)
      {
        ...
        //DLPreprocessParam为预处理参数句柄，推测是根据预处理参数对本次图像进行预处理
        ...
        ho_Image.Dispose();
        HOperatorSet.GetDictObject(out ho_Image, hv_DLSampleBatch, "image");
        ho_SegmentationImage.Dispose();
        HOperatorSet.GetDictObject(out ho_SegmentationImage, hv_DLResultBatch, "segmentation_image");
        //通过语义分割标记的序号作为灰度值，得到各种类型的区域，若该类型没有的话，则区域为0
            ...
            ))-1); hv_ClassIndex = (int)hv_ClassIndex + 1)
        {
              ...
              0))) != 0)
          {
            ...
            HOperatorSet.Intersection(ho_ClassRegion, ho_ROI_0, out ho_RegionIntersection
                );
            ...
            HOperatorSet.SelectShape(ho_ConnectedRegions, out ho_SelectedRegions, 
                "area", "and", hv_MinArea.TupleSelect(hv_ClassIndex-1), 500000000000000);
            }
            hv_Area.Dispose();hv_Row.Dispose();hv_Column.Dispose();
            ...
            {
            HTuple 
              ExpTmpLocalVar_AreaTotals = hv_AreaTotals+(new HTuple(hv_Area.TupleLength()
                ));
            hv_AreaTotals.Dispose();
            hv_AreaTotals = ExpTmpLocalVar_AreaTotals;
            }
            }
            ...
            if (HDevWindowStack.IsOpen())
            {
              HOperatorSet.DispObj(ho_RegionFillUp, HDevWindowStack.GetActive());
            }
        ...
        //显示
        if ((int)(new HTuple(hv_AreaTotals.TupleGreater(0))) != 0)
        {
          //找到目标
          set_display_font(hv_WindowHandle, 30, "mono", "true", "false");
          if (HDevWindowStack.IsOpen())
          {
            HOperatorSet.DispText(HDevWindowStack.GetActive(), "NG", "window", 400, 
        ...
        {
          //通过面积和ROI过来后未找到目标*Do something
          set_display_font(hv_WindowHandle, 30, "mono", "true", "false");
          if (HDevWindowStack.IsOpen())
          {
            HOperatorSet.DispText(HDevWindowStack.GetActive(), "OK", "window", 400, 
                400, "green", new HTuple(), new HTuple());
          }
        }
      ...
      //图片进行压缩后，优化ROI显示问题
      //get_image_size (Object, Width_ZoomImage, Height_ZoomImage)
      //get_image_size (Image, Width_OrigImage, Height_OrigImage)
      //Height_scale := Height_ZoomImage/Height_OrigImage
      //Width_scale := Width_ZoomImage/Width_OrigImage
      if (HDevWindowStack.IsOpen())
      {
        //dev_set_draw ('margin')
      }
      //Part:减去背景类后，通过面积过滤小的
      //select_obj (ClassRegions, ObjectSelected, 1)
      //complement (ClassRegions, RegionComplement)
      //connection (RegionComplement, ConnectedRegions)
      //region_features (ClassRegions, 'area', Areas)

      //for Index1 := 0 to |Areas| by 1
