...
namespace ViewWindow.Model
{
    ...
    public class HWndCtrl
    {
        ...
        public double ImgRow1, ImgCol1, ImgRow2, ImgCol2;//20191030修改这里，为了移动图像到记录位置
        ...
        public void DispObj(HObject hObj, string color, string draw = "margin")
        {
            lock (this)
            {
                ...
                if (hObj != null && hObj.IsInitialized())
                {
                    ...
                    //
                    hObjectList.Add(new HObjectWithColor(temp, color, draw));    //加填充区域
                    HOperatorSet.SetDraw(viewPort.HalconWindow, draw);	//加填充区域
    ...
    HTuple hv_Bold, HTuple hv_Slant)
        {
            ...
            try
            {
                ...
                {
                    {
                        HTuple ExpTmpLocalVar_Font = (((hv_Font_COPY_INP_TMP + "-") + hv_Style) + "-") + hv_Size_COPY_INP_TMP; //随上面注释
        ...
        public void clearHObjectList()
        {
            try
            {
                ...
                foreach (HTextWithColor hTupleWithColor in hTextList)   //释放文本信息
                {
